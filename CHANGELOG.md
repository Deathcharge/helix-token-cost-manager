...
- Generic effective-dated provider/SKU pricing and immutable non-token charge accounting with exact decimal pricing quantities, idempotency, allocation reports, and combined budget spend.
- SQLite schema `1`/`2`/`3` to `4` migration and portable ledger version `2` with backward-compatible version `1` import defaults.
